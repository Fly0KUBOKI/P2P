...
// SoftAP を使って UDP 受信し、受信データをシリアルに表示する (受信のみ、送信は行わない)
  // if (remote != lastRemote || port != lastPort) {
...
static void udpTask(void* pvParameters) {
  ...
  for (;;) {
    ...
    while (remaining > 0) {
      size_t toRead = remaining > (bufSize - 1) ? (bufSize - 1) : remaining;
